...
`card:select` — сохраняет выбранный товар в модель каталога  
`products:selected` — открывает модальное окно с деталями выбранного товара  
`card:preview:button-click` — добавляет или удаляет товар из корзины (в зависимости от текущего состояния)  
...
`basket:submit` — очищает данные заказа и открывает первый шаг оформления (выбор оплаты + адрес)  
...
`order:submit` — переходит ко второму шагу оформления (email + телефон)  
`contacts:submit` — собирает данные заказа, отправляет на сервер, очищает корзину, показывает окно с подтверждением успешного заказа  
`success:close` — закрывает окно с подтверждением успешного заказа  
